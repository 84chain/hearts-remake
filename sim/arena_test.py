...
def print_results(results):
    ...
    print("Average AI points:",
          sum(results["total_ai_points"]) / (iterations * len([i for i in results["ai_points"] if i])),
          "avg stdev of mean: ",
          round(sum([statistics.stdev(i) for i in results["ai_points"] if i]) / (math.sqrt(iterations) * 4), 2))
    print()
